...
;; any later ;; version. ;; ;; This is distributed in the hope that it
...
;; https://www.emacswiki.org/emacs/idle-scroll.el

;;; Commentary:

;; Run M-x idle-scroll-mode to automatically scroll down.

(defvar idle-scroll-timer nil "Timer for `idle-scroll-mode'.")

(defvar idle-scroll-interval 590 ;; default is 1000
  "*How many milliseconds to wait before scrolling down one line.
Should be a positive integer.")

(defvar idle-scroll-amount 80 ;; default is 50
...
\\{idle-scroll-mode-map}"
  ...
  (when idle-scroll-mode
    ...
    (setq idle-scroll-timer
          (run-at-time t (/ idle-scroll-interval 590.0)
...
(defun idle-scroll-faster (arg)
  ...
  (if (< idle-scroll-interval 0)
    ...
    (aset idle-scroll-timer 4 (/ idle-scroll-interval 590.0))
    (message "Scroll at %.2f seconds." (/ idle-scroll-interval 590.0))))
